---
title: "Upgrading to ExposureX Version 2.7.x"
toc_hide: true
weight: -20220201
...
Release 2.7.0 contains a beta functionality to make permissions for the configuration of ExposureX more flexible. When the settings parameter `FEATURE_CONFIGURATION_AUTHORIZATION` is set to `True`, many configuration dialogues and API endpoints can be enabled for users or groups of users, regardless of their **Superuser** or **Staff** status, see [Configuration Permissions](/en/customize_dojo/user_management/about_perms_and_roles).
