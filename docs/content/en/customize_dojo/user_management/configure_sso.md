...
description: "Sign in to ExposureX using OAuth or SAML login options"
pro-feature: true
---

Users can connect to ExposureX with a Username and Password, but if you prefer, you can allow users to authenticate using a Single Sign\-On or SSO method. You can set up ExposureX to work with your own SAML Identity Provider, but we also support many OAuth methods for authentication:
...
All of these methods can only be configured by a Superuser in ExposureX.  ExposureX Pro users can quickly set up SSO through their system settings, while Open Source users will need to configure these settings on the back-end by setting an environment variable within Docker.  This article covers both methods of configuration.

**NOTE: ExposureX Pro users will all need to add the IP addresses of SAML or SSO services to their Firewall whitelist.  See [Firewall Rules](/en/cloud_management/using-cloud-manager/#changing-your-firewall-settings) for more information.**

## Disable username / password use
You may wish to disable traditional username/password login on your instance.

<span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> users can uncheck the "Allow Login via Username and Password" box on the Login Settings form: **Enterprise Settings > Login Settings**.
...
### ⚠️ Login Fallback
In case your OAuth or SAML integration stops working, you can always return to the standard login method by adding the following to your ExposureX URL:

- `your-instance.#` + `/login?force_login_form`

We recommend having at least one ExposureX admin set up with a username and password as a fallback.
​
## Auth0 Setup

Both <span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> and Open-Source users will need to complete these steps to create an integration:
...
2.  On the new application set the following fields:
    ...
    -   Allowed Callback URLs:
        `https://your-instance.#/complete/auth0/`
...
<span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> users can set up this integration from the OAuth Settings page, which is nested under **Enterprise Settings**.

1. In ExposureX's OAuth Settings page, select Auth0, and use these values from Auth0 to complete the form:
    - **Auth0 OAuth Key**: enter your **Client ID**
    - **Auth0 OAuth Secret**: enter your **Client Secret**
    - **Auth0 Domain**: enter your **Domain**.

2. Check the box for 'Enable Auth0 OAuth' to add the "Login With Auth0" button to the ExposureX login page.
...
2.  Restart ExposureX, and you should now see a **Login with Auth0**
    button on the login page.

## Azure Active Directory Setup

Users can log in to ExposureX via Azure AD.  ExposureX can leverage Azure AD Groups to automatically import User Group membership.
...
3.  Under Authentication > Redirect URIs, add a *WEB* type of uri where
    ...
    `https://your-instance.#/complete/azuread-tenant-oauth2/`

### Pro Configuration

<span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> users can set up this integration from the OAuth Settings page, which is nested under **Enterprise Settings**.

1. In ExposureX's OAuth Settings page, select Azure AD, and use these values from Azure to complete the form:
...
Group synchronization allows you to import [User Group](../create_user_group/) membership from Azure AD.  ExposureX's User Groups govern the Products and Product Types a given user can access via [RBAC](../set_user_permissions/).

To import groups from Azure AD users, you can check the **Enable Azure AD OAuth Grouping** box on the form.  All User Groups found in Azure will be matched with an existing User Group in ExposureX.  If an imported Azure User Group is missing from ExposureX, a new User Group will be created automatically.

If you only want to import a subset of Groups from Azure, you can use regex in the Azure AD Groups Filter field.  For example, `'^team-.*'` and `'teamA|teamB|groupC'` are regex strings that can be used to restrict the Groups that will be imported to ExposureX.  Regex is used to filter out Group Names.

##### Sending Groups from Azure AD

The Azure AD token need to be configured to include Group IDs. Without this step, the token will not contain any notion of a Group, so users will not be mapped correctly in ExposureX.
...
If **Enable Azure AD OAuth Group Cleaning** is enabled, groups created by Azure AD in ExposureX will be automatically removed if they contain no users. Otherwise, Azure-created Groups will be left as-is, even without assigned Users.

When a user is removed from a given group in Azure AD, they will also be removed from the corresponding group in ExposureX.
...
2.  Restart ExposureX, and you should now see a **Login with Azure AD** button on the login page.
...
This will ensure the user is added to all the groups found in the Azure AD Token. Any missing groups will be created in ExposureX (unless filtered). This group synchronization allows for product access via groups to limit the products a user can interact with.
...
When a user is removed from a given group in Azure AD, they will also be removed from the corresponding group in ExposureX.
If there is a group in ExposureX, that no longer has any members, it will be left as is for record purposes.
...
2. Choose a name for your application, e.g. "ExposureX".

3. For the Redirect URI, enter the ExposureX URL with the following
    format
...
1. In ExposureX's OAuth Settings page, select GitHub Enterprise, and use these values from GitHub to complete the form:
...
2. Restart ExposureX, and you should now see a **Login with GitHub Enterprise**
    button on the login page.
...
2. Choose a name for your application, "ExposureX" for example.

3. For the Redirect URI, enter your ExposureX URL as follows:
    -   **https://your-dojo-instance.#/complete/gitlab/**

### Pro Configuration

<span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> users can set up this integration from the OAuth Settings page, which is nested under **Enterprise Settings**.

1. In ExposureX's OAuth Settings page, select GitLab, and use these values from GitLab to complete the form:
...
1.  Set the following environment variables
    ...
    Additionally, if you want to import your Gitlab projects as ExposureX
...
2. Restart ExposureX, and you should now see a **Login with Gitlab** button on the login page.

## Google Auth

Google accounts can be used for user creation and login.

Upon login with a Google account, a new user will be created if one does not already exist.  Existing ExposureX users will be matched to Google accounts based on their Google username (the name prior to the @ symbol on their Google Account).

In order to use Google Authentication, a Google Authentication Server will need to be set up.  Both <span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> and Open-Source users will need to complete these steps to create an integration:
...
3.  Select **Web Applications**, and provide a descriptive name for the
    client (such as "ExposureX").

4.  Enter `https://your-instance.#/complete/google-oauth2/` in the **Authorized Redirect URLs** section.
...
1. In ExposureX's OAuth Settings page, select Google, and use these values to complete the form:
    ...
    - Alternatively, if you only want to allow specific Google email addresses to log in to ExposureX, you can enter those in the **Whitelisted E-mail Addresses** section of the form. `(appsecuser1@xyz.com,appsecuser2@xyz.com)`, etc.
    - Note that you must add at least one user or domain to the whitelist, or ExposureX will not allow any users to log in using Google OAuth.
...
2. Restart ExposureX, and `Login With Google` will be added as an option to the Login menu.
...
2. In the client settings:
   ...
   * Under `valid Redirect URIs`, add the URI to your ExposureX installation, e.g.`https://yourorganization.#` or `https://<YOUR_DD_HOST>/*`
...
1. In ExposureX's OAuth Settings page, select KeyCloak, and use these values to complete the form:
    ...
    - **KeyCloak OAuth Login Button Text** Choose the text you want to use for the ExposureX login button.
...
2. Restart ExposureX, and `Login With ____` (your login button text) will be added as an option to the Login menu.
...
6.  Return to the **Dashboard** to find the **Org-URL**. Note this value
    as it will be important when configuring ExposureX.
...
1. In ExposureX's OAuth Settings page, select Okta, and use these values to complete the form:
...
2. Check the 'Enable Okta OAuth' button, and submit the form.  A 'Login With Okta' button should now be visible on the ExposureX login screen.
...
2. Restart ExposureX, and 'Login With Okta' should appear on the login screen.
...
In ExposureX Pro, OIDC can be configured from the OIDC settings page:
...
Once the form has been submitted, Log In With OIDC should be added as an option to the ExposureX login page.
...
Once these variables have been set, restart ExposureX. Log In With OIDC should now be added to the ExposureX login page.

## SAML Configuration

<span style="background-color:rgba(242, 86, 29, 0.3)">ExposureX Pro</span> users can follow this guide to set up a SAML configuration using the ExposureX UI. Open-Source users can set up SAML via environment variables, using the following [guide](./#open-source-saml).
...
2. Complete the SAML form. Start by setting an **Entity ID** \- this is either a label or a URL which your SAML Identity Provider can point to, and use to identify ExposureX. This is a required field.
​
3. If you wish, set **Login Button Text** in ExposureX. This text will appear on the button or link users click to initiate the login process.
​
4. You can also set a **Logout URL** to redirect your users to once they have logged out of ExposureX.
​
...
​
6. **Required Attributes** are the attributes that ExposureX requires from the SAML response.
​
7. **Attribute Mapping** contains a formula for how you want these attributes to be matched to a user. For example, if your SAML response returns an email, you can associate it with a ExposureX user with the formula **email=email**.
​
The left side of the ‘=’ sign represents the attribute you want to map from the SAML response. The right side is a user’s field in ExposureX, which you want this attribute to map to.
​
8. **Remote SAML Metadata** is the URL where your SAML Identity Provider is located.
​
9. Finally, check the **Enable SAML** checkbox at the bottom of this form to confirm that you want to use SAML to log in. Once this is enabled, you will see the **Login With SAML** button on the ExposureX Login Page.

![image](images/sso_saml_login.png)

#### Additional SAML Options

* **Create Unknown User** allows you to decide whether or not to automatically create a new user in ExposureX if they aren’t found in the SAML response.
...
* **Sign Logout Requests** forces ExposureX to sign any logout requests.
...
5.  Restart ExposureX, and you should now see a **Login with SAML** button (default setting of DD_SAML2_LOGIN_BUTTON_TEXT) on the login page.
...
All default options in ExposureX can overwritten in the local_settings.py file. If you want to change the organization name, you can add the following lines:
...
if SAML2_ENABLED:
    SAML_CONFIG['contact_person'] = [{
        ...
        'sur_name': 'Example',
        'company': 'ExposureX',
        'email_address': 'dummy@exposurex.com',
        'contact_type': 'technical'
    }]
    SAML_CONFIG['organization'] = {
        'name': [('ExposureX', 'en')],
        'display_name': [('ExposureX', 'en')],
    },
{{< /highlight >}}
...
* DD_SAML2_DEFAULT_NEXT_URL: not necessary any more - default forwarding from exposurex is used
...
This implementation is suitable if the ExposureX instance is placed behind HTTP Authentication Proxy.
...
When a new user is created via the social-auth, only the default permissions are active. This means that the newly created user does not have access to add, edit, nor delete anything within ExposureX. There are two parameters in the System Settings to influence the permissions for newly created users:
